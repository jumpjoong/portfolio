...
(function (global, factory) {
  ...
  MomentumSlider.prototype = {
    ...
    initHtml: function () {
      this.msContainer = is.str(this.o.el)
      ...
      if (this.o.range) {
        ...
        for (var i = this.o.range[0]; i <= this.o.range[1]; i++) {
          html += is.fnc(this.o.rangeContent)
            ? buildSlide(this.o.rangeContent(i))
            : buildSlide(i);
        }
        html += "</ul></div>";
    ...
    select: function (index) {
      if (this.enabled) {
        ...
        // this.updateSlider();
        this.updateSlider(
          (index + this.o.loop) * (this.o.reverse ? this.step : -this.step)
        );
      }
    },
    ...
    renderTarget: function () {
      if (this.o.sync) {
        ...
        while (syncIndex--) {
          ...
          syncSlider.targetPosition =
            (((syncSlider.o.reverse ? -1 : 1) * this.targetPosition) /
              this.sliderWidth) *
            syncSlider.sliderWidth;
      ...
      var actualIndex =
        ((this.o.reverse ? 1 : -1) * this.targetPosition) / this.step;
      ...
      if (
        !is.und(this.lowerIndex) &&
        this.lowerIndex != lowerIndex &&
        this.lowerIndex != higherIndex
      ) {
        this.setStyle(this.lowerIndex, 1, true);
      }
      if (
        !is.und(this.higherIndex) &&
        this.higherIndex != lowerIndex &&
    ...
    updateAndRender: function () {
      var pointerChange = this.o.vertical
      ...
      if (this.o.bounceCoefficient) {
        ...
        if (diff !== 0) {
          this.targetPosition -=
            pointerChange * dragOutOfBoundsMultiplier(diff) * this.o.multiplier;
        }
      } else {
    ...
    startDecelAnim: function () {
      ...
      var positionOffset = this.o.vertical
    ...
    fixCurrentIndex: function () {
      if (this.o.loop) {
        if (this.currentIndex < this.o.loop) {
          this.currentIndex =
            this.sliderLength - this.o.loop + (this.currentIndex - this.o.loop);
        } else if (this.currentIndex > this.sliderLength - this.o.loop - 1) {
          this.currentIndex =
            this.currentIndex + this.o.loop * 2 - this.sliderLength;
        }
      }
    },
    updateSlider: function (newTargetPosition, initial) {
      if (is.und(newTargetPosition)) {
        newTargetPosition =
          (this.o.reverse ? 1 : -1) * this.currentIndex * this.step;
      } else {
        this.currentIndex =
          ((this.o.reverse ? 1 : -1) * newTargetPosition) / this.step;
      }
        ...
        function (progress) {
          _.targetPosition =
            to > from
              ...
              : from - (from - to) * progress; // 0 - ((0 - -2100) * progress)
          ...
          if (
            ...
            ((_.targetPosition > sliderMax &&
              _.targetPosition >
                sliderMax +
                  Math.min(
                    (to - sliderMax) * _.o.bounceCoefficient,
                    _.o.bounceMax
                  )) ||
              (_.targetPosition < sliderMin &&
                _.targetPosition <
                  sliderMin -
                    Math.min(
                      -(to - sliderMin) * _.o.bounceCoefficient,
                      _.o.bounceMax
                    )))
          ) {
            _.animateInstance.stop();
            _.animateTarget(
              _.targetPosition < sliderMin ? sliderMin : sliderMax,
              false,
              true
            );
            _.currentIndex =
              _.targetPosition < sliderMin ? 0 : _.sliderLength - 1;
          } else {
            _.renderTarget();
          }
        },
    ...
    onChangeCurrentIndex: function (index) {
      ...
      currentIndex =
        currentIndex === this.sliderLength - this.o.loop * 2 ? 0 : currentIndex;
  ...
  function getCurrentValue(values, diff, lower) {
    ...
    var diffValue = lower
      ? centerValue - lowerValue
      : centerValue - higherValue;
    return lower
      ? centerValue - diffValue * diff
      : centerValue + diffValue * diff;
  }
